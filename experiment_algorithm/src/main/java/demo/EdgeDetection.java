...
public class EdgeDetection {
			...
			for (int j = 0; j < 8; j++) {
				...
				for (int x = scale; x < image.getWidth() - scale; x++) {
					for (int y = scale; y < image.getHeight() - scale; y++) {
						ArrayList<Point> q = diAvgAlgo ? getPointsWithPositionDIHat(new Point(x, y), theta, scale) : getPointsWithPosition(new Point(x, y), theta, scale);
						int sum = 0;
						for (Point p : q)
	...
	public ArrayList<Point> getPointsWithPosition(Point p, double theta, int scale) {
		...
		// lookup table would be more efficient here, if scale is 5, then take
		// the calculation
	...
	/*
	 * public static void main(String[] args) { EdgeDetection ed = new
	 * EdgeDetection(); ed.debugger(); }
	 */
}
